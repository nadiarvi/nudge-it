...
export function TaskCard({
  ...
  useEffect(() => {
    const isAssignee = uid === assignedTo._id;
    const isReviewer = reviewer?.length ? uid === reviewer[0]._id : false;
    ...
    if (isAssignee) {
      ...
      options = ['To-Do', 'In Review', 'Revise', 'Done'];
    }
    setStatusOptions(options);
  }, [status, reviewer, assignedTo, uid]);
